...
METRICS = [
    ("agent0/flight_disk_projection_ratio", "Flight actions projected onto legal disk", 100.0, "%"),
    ("agent0/flight_proposal_norm_mean", "Mean proposed flight-action norm", 1.0, "normalized action units"),
]
...
for metric_index, (tag, title, scale, unit) in enumerate(METRICS):
...
fig.suptitle("Recent strict-1+5 experiments: flight-action diagnostics", y=0.985)
fig.text(0.5, 0.89, "Faint: raw records; solid: trailing 2.5M-step mean; dots: current endpoints",
         ha="center", fontsize=8, color="0.35")
fig.legend(legend_handles, legend_labels, loc="lower center", bbox_to_anchor=(0.5, 0.005),
           ncol=3, frameon=False)
fig.subplots_adjust(top=0.78, bottom=0.24, wspace=0.28, left=0.10, right=0.98)
