# Figure Audit

- script_ran: yes
- png_exists: yes
- chart_type_matches_spec: pass
- data_columns_match_spec: pass
- axis_scales_match_spec: pass
- labels_units_match_spec: pass
- series_category_order_match_spec: pass
- legend_complete_and_uncropped: pass
- annotations_match_spec: pass
- forbidden_elements_absent: pass
- obvious_text_overlap_or_clipping: pass
- repairs_made: moved long metric names from overlapping y-axis labels to panel titles and shortened the y-axis labels
- remaining_warnings: bundled validator falsely treats indented axis subfields as new top-level fields; code and rendered output were checked manually
- final_status: REPAIRED
