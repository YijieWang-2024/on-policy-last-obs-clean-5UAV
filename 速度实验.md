...
> 当前归档刷新：2026-08-13 15:24:15；远端 3090 的 6 个正式 run 已完整同步到本地结果树，最新曲线与统计均基于这次归档。
...
- `v_max=10/20/30` 的 seed32、seed42，以及 `v_max=40` 的 seed2/32/42 已完成到 `59.9808M`；本地 v10/20/30 的 seed2 在接近末尾前被提前终止。
- 最新图在 `59.9808M` 的可用 seed 终点为：v10 `491,349`（2/3）、v20 `497,131`（2/3）、v30 `563,534`（2/3）、v40 `559,710`（3/3）。
- 完整 seed 的 tail100 均值为：v10 `485,026`、v20 `496,693`、v30 `562,608`（各为 seed32/42）；v40 三 seed 为 `558,999`。因此 v30 与 v40 都明显高于 v10/v20，但 v30 当前缺少 seed2 的完整末段，不能把 v30 的两 seed 结果冒充三 seed 完整结论。
- 四种速度共同达到的最后步数为 `51.7376M`；在这个严格共同预算点，v30 为 `553,917 ± 2,867`，v40 为 `553,006 ± 4,972`，两者非常接近。
- 结合 `Delta_t=0.5 s`，单 slot 最大位移为 `0.5·v_max`；本轮结果足以说明速度从 10/20 提升到 30/40 有明显收益，但 v30 与 v40 的最终三 seed 排名仍需补齐 v10/20/30 的 seed2 完整预算后再定。

### 1.4 2026-08-13 15:16 最终归档状态

已重新运行同步脚本并完成远端六个正式 run 的最终归档。远端 v10/20/30 的 seed32 和 v40 的 seed2/32/42 均为 `59.9808M`；本地 v10/20/30 的 seed2 分别在 `57.6768M/57.5744M/51.7376M` 停止。当前图因此对每个速度按“每一步实际可用 seed”聚合，尾部对 v10/20/30 使用 2 个完成 seed，v40 使用 3 个完成 seed。
...
| `v_max` | seed | 设备 | 正式结果文件夹名 | 当前状态（2026-08-13 15:24） |
|---:|---:|---|---|---:|
| 10 | 2 | 本地 | [`dcppoR520_fixed600_200_layoutctx_inputv2_peragentnoise_s3p0_md12_vmax10_seed2_60m_20260812`](onpolicy/scripts/results/mec/mappo/dcppoR520_fixed600_200_layoutctx_inputv2_peragentnoise_s3p0_md12_vmax10_seed2_60m_20260812/run1/) | 57.6768M，提前终止 |
...
| 20 | 2 | 本地 | [`dcppoR520_fixed600_200_layoutctx_inputv2_peragentnoise_s3p0_md12_vmax20_seed2_60m_20260812`](onpolicy/scripts/results/mec/mappo/dcppoR520_fixed600_200_layoutctx_inputv2_peragentnoise_s3p0_md12_vmax20_seed2_60m_20260812/run1/) | 57.5744M，提前终止 |
...
| 30 | 2 | 本地 | [`dcppoR520_fixed600_200_layoutctx_inputv2_peragentnoise_s3p0_md12_vmax30_seed2_60m_20260812`](onpolicy/scripts/results/mec/mappo/dcppoR520_fixed600_200_layoutctx_inputv2_peragentnoise_s3p0_md12_vmax30_seed2_60m_20260812/run1/) | 51.7376M，提前终止 |
...
| 40 | 2 | 远端 3090 | [`dcppoR520_fixed600_200_layoutctx_inputv2_peragentnoise_s3p0_md12_vmax40_seed2_60m_20260812`](onpolicy/scripts/results/mec/mappo/dcppoR520_fixed600_200_layoutctx_inputv2_peragentnoise_s3p0_md12_vmax40_seed2_60m_20260812/run1/) | 59.9808M，完成 |
| 40 | 32 | 远端 3090 | [`dcppoR520_fixed600_200_layoutctx_inputv2_peragentnoise_s3p0_md12_vmax40_seed32_60m_20260812`](onpolicy/scripts/results/mec/mappo/dcppoR520_fixed600_200_layoutctx_inputv2_peragentnoise_s3p0_md12_vmax40_seed32_60m_20260812/run1/) | 59.9808M，完成 |
| 40 | 42 | 远端 3090 | [`dcppoR520_fixed600_200_layoutctx_inputv2_peragentnoise_s3p0_md12_vmax40_seed42_60m_20260812`](onpolicy/scripts/results/mec/mappo/dcppoR520_fixed600_200_layoutctx_inputv2_peragentnoise_s3p0_md12_vmax40_seed42_60m_20260812/run1/) | 59.9808M，完成 |
...
当前最终归档状态（同步完成于 2026-08-13 15:16）：

| run | 本地完整归档 | 本次归档事件进度 | 状态 |
|---|---|---:|---|
...
| v40 / seed2 | [`run1/`](onpolicy/scripts/results/mec/mappo/dcppoR520_fixed600_200_layoutctx_inputv2_peragentnoise_s3p0_md12_vmax40_seed2_60m_20260812/run1/) | 59.9808M | 最终归档 |
| v40 / seed32 | [`run1/`](onpolicy/scripts/results/mec/mappo/dcppoR520_fixed600_200_layoutctx_inputv2_peragentnoise_s3p0_md12_vmax40_seed32_60m_20260812/run1/) | 59.9808M | 最终归档 |
| v40 / seed42 | [`run1/`](onpolicy/scripts/results/mec/mappo/dcppoR520_fixed600_200_layoutctx_inputv2_peragentnoise_s3p0_md12_vmax40_seed42_60m_20260812/run1/) | 59.9808M | 最终归档 |
...
运行后按提示输入远端密码即可。脚本会把六个远端 run 同步到本地结果树，完成相对路径/文件大小校验，并生成 [`remote_speed_sync_manifest.json`](onpolicy/scripts/remote_speed_sync_manifest.json)；脚本不会删除远端文件。
...
| 40 | 59.9808M | 3 | 559,710 | 1,396 |

注意：这是最新图各曲线“最后一个可用记录点”的图例值，不是更稳健的 tail-window 统计。v10/20/30 的 seed2 提前终止，所以它们在 `59.9808M` 的终点只使用 seed32/42；v40 使用三个已完成 seed。

### 6.2 完整预算 seed 的 tail100 结果（含提前停止 seed 参考）

| `v_max` | seed2 tail100（提前终止） | seed32，远端 tail100 | seed42，本地 tail100 | 可用 seed 均值 | population std |
|---:|---:|---:|---:|---:|---:|
| 10 | 482,877（57.6768M） | 474,290 | 495,761 | **485,026**（2） | 10,736 |
| 20 | 534,887（57.5744M） | 489,926 | 503,460 | **496,693**（2） | 6,767 |
| 30 | 557,886（51.7376M） | 562,101 | 563,115 | **562,608**（2） | 507 |
| 40 | 558,452 | 562,266 | 556,279 | **558,999**（3） | 2,475 |

可用 seed 均值和 population std 只统计已达到 `59.9808M` 的完整 seed；v10/20/30 的 seed2 仅作为提前停止时的最后 100 点参考，特意标出其步数，不能与完整 60M tail100 混称。

### 6.3 四速度共同早期预算点

四个速度的三个 seed 都达到的最后共同预算为 `51.7376M`。该点的三 seed 均值 ± population std 为：
...
共同预算下 v30 与 v40 接近；这提供了严格同预算比较，但不能代替 v10/20/30 seed2 补齐后的最终三 seed 统计。
...
1. 远端 v40 的 seed2/32/42 已全部达到完整 `59.9808M`，六个远端 run 已同步并通过文件清单校验。
2. 本地 v10/v20/v30 的 seed2 已提前终止，因此当前尾部统计对这三个速度只使用 seed32/42；若需要严格的四速度三 seed 完整预算排名，需要补跑或续跑这三个 seed2。
3. 当前可以稳健地报告“30/40 明显优于 10/20”，但不能把 v30 的两 seed tail100 与 v40 的三 seed tail100 当成完全同等的最终三 seed 证据。
...
参数一致性审计：通过
图片 QA：通过
远端正式实验归档：6 个；12-run 速度矩阵目录：已记录
当前最强曲线：v30 两个完整 seed 的 tail100 均值 562,608；v40 三 seed 为 558,999
结论边界：30/40 明显优于 10/20；v30 的 seed2 未完成，严格四速度三 seed 最终排名待补齐
```
