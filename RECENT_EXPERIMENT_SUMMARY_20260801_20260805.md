# 近一周实验总结与交接（2026-08-01 至 2026-08-09）

> 本文是 8 月 1 日至 9 日实验的唯一周级交接入口。它依据三条 Codex 任务线的对话记录、仓库内保存的 TensorBoard 导出、CSV/JSON 汇总、训练参数和评估图重新整理。第 9 节的旧 9001/9012 证据仍来自本地归档；第 10 节则在 2026-08-09 对新 3090（114.212.117.24）进行了实时只读复核，并与本地事件文件共同重算。
>
...
| **错误设置/隔离** | 协议或关键参数与目标基准不一致；只保留故障证据，禁止用于算法排名、热启动和绘图 |
...
- 本文件：8 月 1–9 日的整理后周总结；第 1–8 节保留 8 月 1–5 日的历史交接，第 9 节覆盖 8 月 6–8 日的旧 18 路远程批次，第 10 节覆盖 8 月 7–9 日的新 3090 噪声、带噪声半径与 Random12 input-v2 批次；若旧文档章节与本文件冲突，以本文件和对应原始 CSV/JSON 为准。
...
## 10. 2026-08-07–08-09 固定布局优势噪声与带噪声通信半径审计

### 10.1 项目经理结论：哪些判断成立

1. **周五 16:50–17:16 的首批非 `A_` 0.2/0.4/0.8 是错误启动实验。** 它们不只是热点区域从基准的 175 m 改成了 200 m；Actor 消息、消息池、Spatial Flight、Cartesian Flight、用户排序、UAV 初始位置和若干启动参数也同时改变。三条曲线只能说明那个错误协议内部的训练现象，禁止拿来与无噪声基准或后续噪声组排名。
2. **周五 22:12 后的 `A_` 组、远端 1.4/2.0，以及周六 2.5/2.66/2.83/3/4/8/16/30，空间环境确实都是固定的 175 m × 175 m 小区域和 400 m × 400 m 大区域。** 它们的目标是只改变 `noise_scale`；但 0.4/0.8 的长运行 `run6` 又被 `D_min=200` 污染，已单独隔离。
3. **噪声 3 的 R0/R260/R520/R780 是参数合同一致的“端到端半径扫描”，但不是纯 Actor 通信单因素消融。** 四份 `args.json` 全字段对照后，除 `experiment_name` 外，实际只变化了 `neighbor_distance` 和 `neighbor_R`，且二者分别同时设为 0/260/520/780。不过 `actor_neighbor_obs=false`，所以 `neighbor_R` 对这批运行没有进入有效代码路径；真正起作用的是 `neighbor_distance`。
4. **`neighbor_distance` 在当前合同中同时控制三个机制。** 它控制 Actor `task_summary` message 的距离门、k-hop Critic state/attention mask，以及 rollout 末端位置形成的 Metropolis 共识图。后者又决定每架 UAV 的有效噪声系数。因此该实验回答的是“整个 DCPPO-Rx 联合机制是否改善”，不能把 R260 相对 R0 的全部差异写成 Actor 通信收益。
5. **8 月 9 日的 Random12 input-v2 是另一套协议。** 它使用 200 m × 200 m 小区域、12 种 episode 布局、`episode_layout_context=true`、`meters_v2` 和 `absolute_raw_v2`；不能与固定 175 m 半径组直接合并排名。

### 10.2 正式固定布局合同

除噪声幅度或通信半径外，后续有效实验共同使用：

| 项目 | 固定设置 |
|---|---|
| 地图与热点 | 600 m × 600 m；`fixed_legacy`；小区域 `[0,175]×[0,175]`，大区域 `[200,600]×[200,600]`；每个 episode 都固定 |
| 动态 MD | 每 slot 小区域均匀生成 1 个候选、大区域均匀生成 4 个候选；出生时在任一 UAV 120 m 覆盖内才接纳；lifetime=10 |
| MD 任务 | `D∼Uniform[200000,400000]`，`C∼Uniform[675M,800M]`，delay∈[0.499,0.5] s；平均移动速度 3 m/s |
| UAV | 5 架；显式初始坐标 `(110,180),(220,180),(330,180),(440,180),(400,400)`；无 reset curriculum |
| Actor | Cartesian Spatial Flight；`task_summary + receiver_gated_sum`；completion-priority；`actor_neighbor_obs=false` |
| Critic/训练 | ego-query attention critic；共享 return normalization；64 rollout workers；episode_length=400；seed=2；目标 60M |
| 优势噪声 | `A_i^train = A_i + m_i × noise_scale × σ_A × ε_i`；`σ_A` 是同一时刻、同一 rollout 环境下五架 UAV 优势的标准差，`ε_i∼N(0,1)` 独立采样；**不加 `A_mean`** |
| 噪声系数 `m_i` | `clip(|A_i^cons−A_mean| / |A_i−A_mean|, 0, 1)`；R0 的 self-only 图使 `m_i=1`；R>0 使用 rollout 最后一步、reset 前保留的 UAV 位置建图并做 50 轮 Metropolis 混合 |
| R0 | `neighbor_distance=0`，`neighbor_R=0`；Actor message 为零、Critic 只激活自身、`m_i=1` |
| R260/R520/R780 | `neighbor_distance=neighbor_R=260/520/780`，`n_iterations=50`；其中 `neighbor_R` 因 `actor_neighbor_obs=false` 实际不生效 |

需要特别区分：候选 MD 在矩形内是均匀出生，但“活跃 MD”还经过 UAV 120 m 覆盖准入，因此活跃样本的空间分布会受到策略当前位置影响。

### 10.3 错误设置与无效碎片

| 标签 | 实验 | 最后 step | 错误/限制 | 处理 |
|---|---|---:|---|---|
| **ERROR-WRONG-PROTOCOL** | 非 `A_` noise 0.2/0.4/0.8 | 18.15M / 17.69M / 16.26M | `episode_template4_600_200,index=0`，即 200 m 小区；`md_arrivals_min/max=6/6`；message disabled；mean pool；无 Spatial/Cartesian Flight；无 completion-priority；UAV 起点未显式匹配 | **隔离，禁止与正式基准比较** |
| 诊断碎片 | `A_` noise 0.2 run1 | 1.10M | 协议正确但预算太短 | 只保留启动验证，不作性能结论 |
| 诊断碎片 | `A_` noise 0.4 run4 | 0.59M | 协议正确但预算太短 | 同上 |
| 诊断碎片 | `A_` noise 0.8 run5 | 0.38M | 协议正确但预算太短 | 同上 |
| **ERROR-DMIN-200** | `A_` noise 0.4 run6 | 6.73M | `D_min=200`，而不是 200000 | **隔离**；tail100=412.2k 不得使用 |
| **ERROR-DMIN-200** | `A_` noise 0.8 run6 | 52.04M | 同上 | **隔离**；tail100=487.1k 不得使用 |

这解释了旧图中曾出现的上下文污染：周五 21:27 的 200 m 轨迹属于第一行；后来某些带基准曲线图又错误读取了最后两行的 `run6`。本节新图把这五条全部排除。

### 10.4 噪声幅度：最新训练进展与公平窗口

下表的 `tail100` 是各自事件末端值，用于记录进展，**由于最终预算不同，不能直接当作最终排名**。

| noise | 设备 | 最后 step | tail100 True60 | 状态 |
|---:|---|---:|---:|---|
| 0（无噪声基准） | 历史归档 | 59.98M | 486.7k | 完成 |
| 1.4 | 远端3090 | 45.80M | 484.7k | 按计划停止 |
| 2.0 | 远端3090 | 47.39M | 483.4k | 按计划停止 |
| 2.5 | 本地3090 | 18.15M | 469.9k | 按计划停止 |
| 2.66 | 远端3090 | 56.65M | 425.6k | 近完成但当前不在运行 |
| 2.83 | 本地3090 | 8.63M | 381.5k | 按计划停止 |
| 3.0 | 本地3090 | 59.98M | 426.1k | 完成 |
| 4.0 | 远端3090 | 38.43M | 411.8k | 按计划停止 |
| 8.0 | 远端3090 | 29.57M | 394.7k | 按计划停止 |
| 16 | 本地3090 | 8.88M | 291.6k | 按计划停止 |
| 30 | 本地3090 | 8.68M | 239.4k | 按计划停止 |

为了消除预算差异，图的下半部分统一截到所有有效曲线均已达到的 **8.6272M**，计算截点前 tail25：

| noise | 0 | 1.4 | 2.0 | 2.5 | 2.66 | 2.83 | 3.0 | 4.0 | 8.0 | 16 | 30 |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| True60 | 444.9k | 451.3k | 453.4k | 447.9k | 393.0k | 391.1k | 391.3k | 384.1k | 359.6k | 307.2k | 255.2k |
| 相对无噪声 | 0% | +1.45% | +1.90% | +0.68% | −11.68% | −12.09% | −12.05% | −13.66% | −19.17% | −30.94% | −42.63% |

![固定布局噪声幅度训练曲线与共同预算比较](paper_artifacts/noise_radius_20260809/noise_sweep/plot.png)

**当前能支持的判断：**

- 0–2.5 在同一早期预算内处于同一性能档，差异不超过约 2%；不能声称小噪声有稳定增益。
- 2.5→2.66 出现约 **−12.28%** 的突降；2.66、2.83、3.0 又高度接近，说明此前观察到的“2.5 和 3 突变”不是单纯画图错觉。
- 1.4/2.0 在 45–47M 仍接近无噪声基准，而 2.66/3.0 在 56–60M 仍停留在约 426k，说明两档差异不是只有早期学习速度造成。
- 仍然只有 seed=2。最严谨的项目表述是“存在 2.5–2.66 附近的候选优化阈值”，而不是“噪声 2.66 必然导致相变”。下一步应复现 2.5、2.66 和 3.0 的独立 seed，不再继续切更多小数点。

### 10.5 噪声 3 下的通信半径

四个运行的完整参数差异核对结果为：除了实验名，只变化了 `neighbor_distance` 和 `neighbor_R`；但后者因 `actor_neighbor_obs=false` 不生效。`neighbor_distance` 同时进入 Actor message、Critic state 和优势噪声的共识残差，故本节是联合机制比较。各自末端进展如下：

| 半径 | 最后 step | 末端 tail100 | 状态 |
|---:|---:|---:|---|
| R0 | 59.98M | 426.1k | 完成 |
| R260 | 46.52M | 490.7k | 当前不在运行，未满 60M |
| R520 | 46.57M | 494.1k | 当前不在运行，未满 60M |
| R780 | 56.50M | 497.2k | 近完成但当前不在运行 |

新图同时给出完整曲线和 **46.5152M** 共同预算 tail100：

| 半径 | R0 | R260 | R520 | R780 |
|---:|---:|---:|---:|---:|
| 共同预算 True60 | 419.1k | 490.7k | 494.1k | 494.3k |
| 相对 R0 | 0% | +17.07% | +17.89% | +17.95% |
| `noise_magnitude_mean` tail100 | 1.0 | 1.45e−5 | 2.17e−6 | 1.90e−6 |

![噪声3下的通信半径训练曲线与共同预算比较](paper_artifacts/noise_radius_20260809/radius_noise3/plot.png)

**当前能支持的判断：**

- R0→R260 的原始差异约 17%，但同时 `m_i` 从 1 降到约 `1.45e−5`，即自适应噪声几乎被共识完全消除。**所以这 17% 不能解释为 260 m Actor 通信本身带来的提升。**
- 同一 46.52M 窗口，无噪声 R0 基准的 tail100 为 478.5k；R260/R520/R780 相对它分别约 +2.54%/+3.25%/+3.30%。这更接近“去掉强噪声后，Actor message + Critic 可见范围的联合增益”，但仍不能拆分 Actor 与 Critic 的贡献。
- R260→R520 只增加约 0.70%，R520→R780 约 0.05%，同时三者的平均 `m_i` 都已接近零。因此“R260/R520/R780 性能区分不开”是可靠观察，半径 260 m 后出现联合机制饱和。
- 这仍只有 seed=2，且 R260/R520/R780 未全部跑满 60M。下一轮应先做机制解耦：R260/noise=0、R260/noise=3 但强制 `m_i=1`、以及当前 R260/自适应 `m_i`；否则继续补同合同 seed 只能确认联合算法稳定性，不能回答通信究竟贡献多少。

### 10.6 8 月 9 日 Random12 input-v2 进展

远端实时只读复核时没有发现仍在运行的 `train_mec.py` 进程；三条 Random12 新运行停留在很早期：

| 实验 | 最后事件 step | 状态 | 是否进入本节固定布局图 |
|---|---:|---|---|
| Random12 R0 / noise 3 | 2.48M | 早期中断/当前不运行 | 否 |
| Random12 R260 / noise 3 | 2.28M | 早期中断/当前不运行 | 否 |
| Random12 R520 / noise 3 | 2.18M | 早期中断/当前不运行 | 否 |

它们的热点布局、布局上下文和 Actor message 数值合同都与固定布局组不同，当前数据量也不足以判断半径排序。项目处理应为 `valid-protocol-but-incomplete`，不是错误设置；若继续，应从同一 Random12 input-v2 合同重新完整启动三条并保持独立 seed 记录。

### 10.7 本批次的项目决策与证据入口

- `candidate-noise-threshold`：固定布局下 2.5–2.66 附近可能存在优化阈值；先补 seed，不再追加 2.58/2.60/2.62 一类密集扫描。
- `candidate-joint-radius-saturation`：noise=3 时 R260 相对 R0 的联合机制优势约 17%，其中主要可见变化是共识把平均噪声系数从 1 压到约 `1.45e−5`；R520/R780 增量很小。先做噪声/Actor/Critic 解耦，再决定是否补多 seed，不继续扩到更大半径。
- `quarantined`：非 `A_` 0.2/0.4/0.8 和 `D_min=200` 的 0.4/0.8 run6 永久排除。
- `separate-protocol`：Random12 input-v2 单独建组，不与固定 175 m 组拼图排名。
- 曲线原始导出：`paper_artifacts/noise_radius_20260809/data/training_curves.csv`。
- 端点、状态、错误标签与 SHA256：`paper_artifacts/noise_radius_20260809/data/endpoints.csv`。
- 半径共同预算的有效噪声系数：`paper_artifacts/noise_radius_20260809/data/radius_noise_magnitude_common_budget.csv`。
- 远端原始路径与完整 `args.json` 快照：`paper_artifacts/noise_radius_20260809/data/remote3090_source_manifest.json`。
- 数据收集脚本：`paper_artifacts/noise_radius_20260809/collect_latest.py`。
- 噪声图复现：`paper_artifacts/noise_radius_20260809/noise_sweep/plot.py`、`figure-spec.md`、`audit.md`。
- 半径图复现：`paper_artifacts/noise_radius_20260809/radius_noise3/plot.py`、`figure-spec.md`、`audit.md`。
