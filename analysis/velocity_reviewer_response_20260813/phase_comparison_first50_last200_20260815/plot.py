...
arrival-before/after split: every metric is summarized over the first 50 slots,
...
PHASES = [
    ("first50", "First 50 slots", "o", "--", slice(0, 50)),
...
METRICS = {
    ...
    "system_energy": {
        "field": "task_plus_flight_energy_increment",
        "label": r"Mean system energy per slot ($\times 10^2$ J)",
        "title": "Phase-wise mean system energy per slot",
        "scale": 1e2,
        "format": "{:.3f}",
    },
}
...
def load_rows():
    ...
    for row in rows:
        ...
        for field in ("system_performance_increment", "task_plus_flight_energy_increment"):
            if row["data"][field].shape != (400,):
                raise RuntimeError(f"unexpected {field} shape in {row['name']}")
    return rows
...
def phase_value(row, metric_key, phase_slice):
    ...
    # Speed is stored per UAV and per slot; the other two quantities are
    # already system-level slot increments.  In both cases the result is one
    # scalar mean for one episode and one fixed phase window.
...
def write_summary(records):
    ...
    with (ROOT / "phase_50_last200_summary.csv").open(
    ...
    lines = [
        ...
        "",
        "- First phase: slots 0--49 (50 slots).",
        ...
        "- System energy is `task_plus_flight_energy_increment`, the per-slot increment of `energy_all_GUs_UAVs` (task plus UAV flight energy).",
    ...
    (ROOT / "phase_50_last200_summary.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
...
def draw_metric(ax, records, metric_key, outside_legend=False):
    ...
    for phase_key, phase_label, marker, linestyle, _ in PHASES:
        ...
        # Three phase lines are deliberately neutral; color encodes v_max at
        # the points, while marker/line style encodes the phase window.
        ax.plot(
            x,
            y,
            color="#4a4a4a",
        ...
        for xv, yv in zip(x, y):
            ax.plot(
                ...
                linestyle="None",
                markersize=6.0,
                markerfacecolor=SPEED_COLORS[int(xv)],
...
def plot_individual(records):
    stems = {
        ...
        "system_energy": "phase_mean_system_energy",
    }
...
def plot_overview(records):
    ...
    for ax, metric_key in zip(axes, METRICS):
        ...
        if ax.artists:
            ax.artists[0].remove()
    speed_handles = [
        Line2D([], [], color=SPEED_COLORS[vmax], marker="o", linestyle="None", markersize=5.2, label=f"{vmax} m/s")
        for vmax in VMAXES
    ]
    phase_handles = [
        Line2D([], [], color="#444444", marker=marker, linestyle=linestyle, markersize=5.0, linewidth=1.1, markerfacecolor="white", markeredgecolor="#444444", label=phase_label)
        for _, phase_label, marker, linestyle, _ in PHASES
    ]
    fig.legend(
        handles=speed_handles + phase_handles,
        labels=[f"{vmax} m/s" for vmax in VMAXES] + [phase[1] for phase in PHASES],
        loc="lower center",
        ncol=6,
        frameon=False,
...
- data_columns: speeds, system_performance_increment, task_plus_flight_energy_increment, positions
...
- y_axis:
  ...
  - label: mean realized UAV speed, mean system performance per slot, or mean system energy per slot
  - scale: linear
  - range: data-driven
- phase_windows: first 50 slots = 0--49; last 200 slots = 200--399; full episode = 0--399
- series_or_categories: three fixed-window phase series across three speed settings
- color_mapping: Okabe-Ito blue for 20 m/s, vermilion for 30 m/s, green for 40 m/s
- marker_mapping: circle for first 50 slots, square for last 200 slots, diamond for full episode
- line_mapping: dashed, solid, and dotted neutral lines connect the three speed settings for the three phase windows
- energy_definition: task_plus_flight_energy_increment, the per-slot increment of energy_all_GUs_UAVs (task plus UAV flight energy)
- aggregation: each plotted point is the mean of 15 episode-level phase means
- legend: speed is encoded by point color; phase is encoded by point marker and line style
...
def write_audit(rows, records):
    ...
    checks = [
        ...
        f"- phase_lengths: first50=50, last200=200, full=400",
        ...
        "- metric_check: pass; speed, system performance increment, and task-plus-flight system energy are all fixed-window means",
        "- color_shape_check: pass; speed uses color and phase uses marker/line style",
        ...
        json.dumps(
            {
                ...
                "records": len(records),
                "figures": sorted(path.name for path in FIG_ROOT.glob("*.png")),
                "summary": str(ROOT / "phase_50_last200_summary.md"),
            },
            ensure_ascii=False,
            indent=2,
        )
