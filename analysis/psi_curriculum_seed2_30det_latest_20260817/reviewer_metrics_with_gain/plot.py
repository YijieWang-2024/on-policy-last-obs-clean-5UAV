"""Combined bar-and-line chart for the latest 25-episode reviewer metrics."""
...
# Default to the new 25-episode evaluation.  The earlier 30-episode results
# remain available through --data-dir and are not overwritten.
DEFAULT_DATA_DIR = (
    ANALYSIS_DIR
    / "results_25det_latest_20260817"
)
BAR_COLORS = ["#0072B2", "#E69F00", "#CC79A7"]
GAIN_COLOR = "#000000"
...
def main() -> None:
    ...
    metric_specs = (
        ...
        ("overall_completion_pooled", "Overall completion rate"),
    )
    ...
    ax.set_ylabel("Rate (%)")
    # Keep the gain line below the near-100% completion bars.
    left_axis_top = 105.0
    ax.set_ylim(0, left_axis_top)
    ax.set_yticks([0, 20, 40, 60, 80, 100])
    ax.grid(axis="y", linestyle=":", linewidth=0.5, alpha=0.7)
    ax.set_axisbelow(True)

    # Use a transparent primary axis so the gain line remains visible over bars.
    ax.set_zorder(1)
    ...
    # Use the actual system-gain scale with the requested fixed range.
    gain_axis_min = 100000.0
    gain_axis_max = 685000.0
    ax2.set_ylim(gain_axis_min, gain_axis_max)
    ax2.set_yticks([100000, 200000, 300000, 400000, 500000, 600000])
    ax.set_title("Offloading, completion, and system gain vs. ψ_min\n(25 deterministic episodes, latest checkpoints)")
