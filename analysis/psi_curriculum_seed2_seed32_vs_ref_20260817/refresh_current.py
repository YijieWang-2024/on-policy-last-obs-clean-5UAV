"""Refresh the three live 3090 runs and make verified local checkpoint copies."""
...
def fetch_run(sftp: paramiko.SFTPClient, key: str, run_name: str) -> dict:
    ...
    event_meta = get_stable(
        ...
        HERE / "data" / "remote" / f"psi{key[3:]}.tfevents",
    )
    ...
    remote_files = [remote_args, remote_manifest] + [remote_model_dir / name for name in expected_files]
    ...
    get_stable(sftp, remote_args, destination_run / "args.json")
    get_stable(sftp, remote_manifest, destination_model / "checkpoint_manifest.json")
    for name in expected_files:
        get_stable(sftp, remote_model_dir / name, destination_model / name)
    ...
    local_manifest = json.loads((destination_model / "checkpoint_manifest.json").read_text(encoding="utf-8"))
    ...
    return {
        ...
        "args": {name: args.get(name) for name in (
            "seed", "association_threshold", "offload_deadline_filter",
            "num_env_steps", "n_rollout_threads", "neighbor_distance", "neighbor_R",
            "hotspot_layout_mode", "md_lifetime_min", "md_lifetime_max", "mean_velocity",
            "noise_scale", "actor_message_mode", "uav_reset_curriculum",
            "uav_reset_curriculum_schedule", "experiment_name",
        )},
    }
