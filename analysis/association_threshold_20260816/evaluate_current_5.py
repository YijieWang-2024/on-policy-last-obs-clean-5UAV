"""Evaluate the current five psi seed-32 runs on ten common deterministic episodes.

The implementation reuses the repository's checked trajectory evaluator.  The
remote models are frozen snapshots made on the 3090 after manifest stability
checks; local live runs are snapshotted once by the evaluator using the same
...
def load_base():
    spec = importlib.util.spec_from_file_location("association_eval_base_20260816", BASE_SCRIPT)
...
def main() -> None:
    ...
    module.RUNS = {
        ...
        "local_psi0p7_deadline_off": {
            ...
            "run_dir": results / "dcppoR520_fixed600_200_layoutctx_noactor_peragentnoise_s3p0_md12_vmax30_psi0p7_nofilter_seed32_60m_20260815" / "run1",
            "snapshot": True,
        },
        "local_psi0p9_deadline_off": {
            "label": "local psi=0.9 | deadline OFF",
            "run_dir": results / "dcppoR520_fixed600_200_layoutctx_noactor_peragentnoise_s3p0_md12_vmax30_psi0p9_nofilter_seed32_60m_20260815" / "run1",
            "snapshot": True,
        },
    }
    module.main()
