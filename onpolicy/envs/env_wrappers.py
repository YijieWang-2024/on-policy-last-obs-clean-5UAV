...
class ShareVecEnv(ABC):
    ...
    def __init__(self, num_envs, observation_space, share_observation_space, action_space, available_actions_space):
        self.num_envs = num_envs
        self.observation_space = observation_space
        self.share_observation_space = share_observation_space
        self.action_space = action_space
        self.available_actions_space = available_actions_space
    ...
    def process_actions(self, actions):
        """
        Process the actions before sending them to environments.
        This allows for action transformation, normalization, or adding noise.

        Args:
            actions: raw actions from the agent

        Returns:
            processed_actions: actions after processing
        """
        pass
...
def shareworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        cmd, data = remote.recv()
        if cmd == 'step':
            ob, s_ob, reward, done, info, available_actions, Metropolis_weights, attention_active_mask = env.step(data)
            if 'bool' in done.__class__.__name__:
                if done:
            ...
            # 调用环境的 process_actions 方法
            processed_actions = env.process_actions(data)
            remote.send(processed_actions)
        elif cmd == 'reset':
            ob, s_ob, available_actions, Metropolis_weights, attention_active_mask = env.reset()
            remote.send((ob, s_ob, available_actions, Metropolis_weights, attention_active_mask))
        elif cmd == 'reset_task':
            ob = env.reset_task()
            remote.send(ob)
        elif cmd == 'render':
...
class ShareSubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        for remote in self.work_remotes:
            remote.close()
        self.remotes[0].send(('get_spaces', None))
        observation_space, share_observation_space, action_space, available_actions_space = self.remotes[0].recv(
        )
        ShareVecEnv.__init__(self, len(env_fns), observation_space,
    ...
    def process_actions(self, actions):
        ...
        """
        for remote, action in zip(self.remotes, actions):
            remote.send(('process_actions', action))
        # 接收处理后的动作
