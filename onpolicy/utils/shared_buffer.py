...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space, available_actions_space=None):
        ...
        self.Metropolis_weights = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, num_agents),
                                           dtype=np.float32)
        self.attention_active_mask = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, num_agents, self.max_UAVs_obs_concat), dtype=np.float32)

        self.available_actions = np.ones(
            (self.episode_length + 1, self.n_rollout_threads, num_agents, available_actions_space), dtype=np.float32)

        if act_space.__class__.__name__ == "Tuple":
            act_shape = 0
            for action_space_i in act_space:
                act_shape += action_space_i.shape[0]
        else:
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, bad_masks=None, active_masks=None, available_actions=None, Metropolis_weights=None, attention_active_mask=None):
        """
        ...
        if Metropolis_weights is not None:
            self.Metropolis_weights[self.step + 1] = Metropolis_weights.copy()
        if attention_active_mask is not None:
            self.attention_active_mask[self.step + 1] = attention_active_mask.copy()
    ...
    def after_update(self):
        ...
        if self.Metropolis_weights is not None:
            self.Metropolis_weights[0] = self.Metropolis_weights[-1].copy()
        if self.attention_active_mask is not None:
            self.attention_active_mask[0] = self.attention_active_mask[-1].copy()
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        if self.attention_active_mask is not None:
            attention_active_mask = self.attention_active_mask[:-1].reshape(-1, self.attention_active_mask.shape[-1])
        ...
        for indices in sampler:
            ...
            attention_active_mask_batch = attention_active_mask[indices]
            ...
            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, \
                ...
                adv_targ, available_actions_batch, attention_active_mask_batch
