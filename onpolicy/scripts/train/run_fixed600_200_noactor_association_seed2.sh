...
if [[ $# -ne 2 ]]; then
  echo "usage: $0 0.3|0.7|0.9 EXPERIMENT_NAME" >&2
  exit 2
fi
...
case "$association_threshold" in
  0.3|0.7|0.9) ;;
  *) echo "association threshold must be 0.3, 0.7, or 0.9" >&2; exit 2 ;;
esac
...
# Deadline filter is intentionally enabled for the remote psi-only comparison.
exec "$python_bin" "$train_script" \
  --env_name mec --algorithm_name mappo --user_name wyj2 --seed 2 --share_policy \
