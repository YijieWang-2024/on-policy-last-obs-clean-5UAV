...
if [[ $# -ne 2 ]]; then
  echo "usage: $0 0.1|0.3|0.5 EXPERIMENT_NAME" >&2
  exit 2
fi

association_threshold="$1"
experiment_name="$2"
case "$association_threshold" in
  0.1|0.3|0.5) ;;
  *) echo "association threshold must be 0.1, 0.3, or 0.5" >&2; exit 2 ;;
esac
...
# This is the no-deadline-filter psi sweep.  Every other argument is kept
# matched to the Fixed600-200 / lifetime-12 / noise-3 / R520 baseline.
exec "$python_bin" "$train_script" \
  --env_name mec --algorithm_name mappo --user_name wyj2 --seed 32 --share_policy \
  ...
  --clip_param 0.15 --gamma 0.99 --ppo_epoch 4 --num_mini_batch 1 \
  --entropy_coef 0 --use_valuenorm --shared_ret_norm \
  --state_is_k_hops --all_uav_k_hops --use_atten_critic --ego_query_critic \
  --local_reward --continuous_associate --association_threshold "$association_threshold" \
  --not_served_rew_to_nearest --cartesian_flight --actor_message_mode disabled \
