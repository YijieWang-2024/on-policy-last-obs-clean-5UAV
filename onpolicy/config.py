...
def get_config():
    ...
    parser.add_argument("--user_name", type=str, default='mappo', help="[for wandb usage], to specify user's name for simply collecting training data.")
    ...
    # ⬇⬇⬇⬇ 这个num_mini_batch要根据episode_length来看。 我现在一个回合长度为50.就分为1个mini_batch
    ...
    # 计算loss损失的时候，使用mean时，只计算活着的agent。 我可以直接设置False就没问题。
    parser.add_argument("--use_value_active_masks",
                        action='store_false', default=True, help="by default True, whether to mask useless data in value loss.")
    # 评估动作的熵的时候使用了一下。action_logits.entropy().mean()还是只计算或者的智能体熵的mean()。mean的范围不一样
    ...
    # save parameters
    parser.add_argument("--save_interval", type=int, default=5, help="time duration between contiunous twice models saving.")
